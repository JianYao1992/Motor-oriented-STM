function [SSQs, DFs, MSQs, Fs, Ps]=mixed_between_within_anova(X,suppress_output,ResultsName)
...
for i=1:n_bs_levels %go through each group(control and experimental group )
    for j=1:n_ws_levels %go through each session (1:25 for five trainging days)
        this_cell_inds=find(all_bs_labels==i & all_ws_labels==j);
        if isempty(this_cell_inds)
            error('At least one empty cell found');
        end
        n_subs_per_cell(i,j)=length(this_cell_inds); %#ok<AGROW>   %get the session number with the same group and session identity
        cell_totals(i,j)=sum(all_dvs(this_cell_inds)); %#ok<AGROW> %sum the performance with the same group and session identity
    end
...
for k=1:n_subjects %go through each mouse
    this_subj_inds=find(all_subj_labels==k);
    if length(this_subj_inds)~=n_ws_levels
        %our second check for this issue
        error('At least one subject missing at least one repeated measure (or is possibly entered more than once)');
    end
    subj_totals(k)=sum(all_dvs(this_subj_inds)); %#ok<AGROW> %sum the performance of all sessions for each mouse
end
...
SStmp=[];
for i=1:n_bs_levels %go through each group(control and experimental group)
    SStmp(i)=(sum(cell_totals(i,:))^2) / sum(n_subs_per_cell(i,:)); %#ok<AGROW> % normalize for each session(1:25)
end
...
if ~suppress_output
    disp(' ');
    disp(' ');
    fileID = fopen([ResultsName '_Results.txt'],'w');
    fprintf(fileID,'     Source                          SSq        df       MSq         F         p        \r\n');
    fprintf(fileID,'-------------------------------------------------------------------------------------\r\n');
    fprintf(fileID,'     Between-subjects factor   %9.3f %9i %9.3f %9.3f %9.7f\r\n',SSbs,DFbs,MSbs,Fbs,Pbs);
    fprintf(fileID,'     Between-subjects error    %9.3f %9i %9.3f\r\n',ERRbs,DFERRbs,MSERRbs);
    fprintf(fileID,'     Within-subjects factor    %9.3f %9i %9.3f %9.3f %9.7f\r\n',SSws,DFws,MSws,Fws,Pws);
    fprintf(fileID,'     Within x between int.     %9.3f %9i %9.3f %9.3f %9.7f\r\n',SSint,DFint,MSint,Fint,Pint);
    fprintf(fileID,'     Within-subjects error     %9.3f %9i %9.3f\r\n',ERRws,DFERRws,MSERRws);
    disp(' ');
    disp(' ');
    fclose(fileID);
end
