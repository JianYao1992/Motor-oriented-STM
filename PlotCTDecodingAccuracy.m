...
Group = 'CtrlGroup';
Target = 'mPFC'; 
NeuronCodingType = 4; % 0: no remove; 1: remove sustained neurons; 2: remove transient neurons 3: only sustained 4: only transient 5: only non-memory neuron
...
hold on 
if exist('ClusterBasedPermuTestIsSig','var')
...
set(gcf, 'Renderer', 'Painter'); saveas(gcf,[Target ' Neurons_' RegionCoding '_CTDecodingConfusionMatrix_' Group File_Recording.name(location1:location2-1)],'fig'); 
% close;
save([Target ' Neurons_' RegionCoding '_ClusterBasedPermuTestIsSigBetterAndLowerthanShuffle_' Group File_Recording.name(location1:location2-1)],'ClusterBasedPermuTestIsSig','ClusterBasedPermuTestIsSigLessChance');
