%% ID of Go-preferred and NoGo-preferred neurons in mPFC and aAIC.
...
BaseLen = 9;
ShownBaseLen = 2;
BasePeriod = (BaseLen-ShownBaseLen)*TimeGain+2:BaseLen*TimeGain+1;
SampOdorLen = 1;
SampPeriod = BaseLen*TimeGain+2:(BaseLen+SampOdorLen)*TimeGain+1;
DelayLen = 4;
TestOdorLen = 0.5;
RespLen = 1;
ShownTrialLen = ShownBaseLen+SampOdorLen+DelayLen+TestOdorLen+RespLen;
WindowSize = 0.1;
SlidingWindow = 0.1;
...
for iReg = 1:numel(Reg)
    load(sprintf('%sSelectivityData_%s.mat',Reg{iReg},Group));
    ...
    RampingAna = zeros(length(FR_TarReg),DelayLen+2);
    RampingAna_Go = zeros(length(FR_TarReg),DelayLen+2);
    RampingAna_Nogo = zeros(length(FR_TarReg),DelayLen+2);
    ...
    for iUnit = 1:length(CodingUnitsID) % assignment of group ID
        ...
        tempSelectivity = tempSelectivity(:,BaseLen+SampOdorLen+1:BaseLen+SampOdorLen+DelayLen);
