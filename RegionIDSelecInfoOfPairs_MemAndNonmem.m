...
for bin = -1:1:5  % from baseline to response period
    ...
    stats = [];
    load(sprintf('TestFC_XCORR_stats_%d_%d_2msbin_%s.mat',bin,bin+1,Group));
    for iPair = 1:length(stats)
        ...
        if isequal(max(a),min(a)) && isequal(max(b),min(b))
            ...
            if isfield(stats{iPair},'AIs1') && ((stats{iPair}.AIs1 ~= 0 && tempreg1 == tempreg2) || (stats{iPair}.AIs1 < 0 && tempreg1 ~= tempreg2))
            ...
            if isfield(stats{iPair},'AIs2') && ((stats{iPair}.AIs2 ~= 0 && tempreg1 == tempreg2) || (stats{iPair}.AIs2 < 0 && tempreg1 ~= tempreg2))
    ...
    %% Divide nonmemory and memory neuron
     % at least one neuron is memory neuron
    a = any(Pref_pair(:,[3 4 5 6 10 11 12 13]),2);
    Pair_reg = Pair_reg(a,:);
    Pair_chain = Pair_chain(a,:);
    Pref_pair = Pref_pair(a,:);
    Pair_mouse = Pair_mouse(a,:);
    Pair_trialnum = Pair_trialnum(a,:);
    a = any(pref_chain_go(:,[3 4 5 6 10 11 12 13]),2);
    ...
    a = any(pref_chain_nogo(:,[3 4 5 6 10 11 12 13]),2);
    ...
    save(sprintf('FCofMemoryNeurons_PropertyPopulation_%d_%d_%s.mat',bin,bin+1,Group),'Pair_reg','Pair_chain','Pref_pair','Pair_mouse','Pair_trialnum',...
    ...
    % two neurons are non-memory neurons
    ...
    save(sprintf('FCofNonmemoryNeurons_PropertyPopulation_%d_%d_%s.mat',bin,bin+1,Group),'Pair_reg','Pair_chain','Pref_pair','Pair_mouse','Pair_trialnum',...
