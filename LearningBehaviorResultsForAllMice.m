...
pValue = GetDataMatrixForMixedRepeatedAnova(C_Perf, E_Perf, strcat('correct rate-',Mani));
p_perf = pValue{1,1};
...
pValue = GetDataMatrixForMixedRepeatedAnova(C_CRRate, E_CRRate, strcat('Hit rate and CR rate-',Mani));
p_CRRate = pValue{1,1};
legend(['Vgat-ChR2 -, n = ' num2str(size(C_Dataofmice,2))],['Vgat-ChR2 +, n = ' num2str(size(E_Dataofmice,2))],'Location','southeast');
legend('boxoff');
set(gca,'XTick',0:1:LearningDay,'XTickLabel',{'0','1','2','3','4','5','6'},'FontName','Arial','FontSize',16,'xlim',[1-0.2 LearningDay+0.2]);
...
close;
pValue = GetDataMatrixForMixedRepeatedAnova(C_dprime, E_dprime, strcat('d prime-',Mani));
p_dprime = pValue{1,1};
...
box off;
pValue = GetDataMatrixForMixedRepeatedAnova(C_EarlyLickRatio, E_EarlyLickRatio, strcat('Early lick-',Mani));
