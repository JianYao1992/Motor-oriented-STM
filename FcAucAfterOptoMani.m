...
% inhibition group
load(sprintf('Functional Coupling Coding_1msbin_%dpairs_%s_NpHRGroup.mat',length(PairsType),Target));
data_inhibition = AllTypesAUC{PairsID};
PairsNuminEachBin_inhibition = cellfun(@(x) size(x,1),data_inhibition,'UniformOutput',1);

FcspAucValue_inhibition{1} = [];
FcspAucValue_inhibition{2} = data_inhibition{2}(:,3);
FcspAucValue_inhibition{3} = data_inhibition{3}(:,3);
FcspAucValue_inhibition{4} = data_inhibition{4}(:,3);


% control group
load(sprintf('Functional Coupling Coding_1msbin_%dpairs_%s_CtrlGroup.mat',length(PairsType),Target));
data_Ctrl = AllTypesAUC{PairsID};
PairsNuminEachBin_Ctrl = cellfun(@(x) size(x,1),data_Ctrl,'UniformOutput',1);
if all(PairsNuminEachBin_Ctrl >= 0)
    FcspAucValue_Ctrl = cellfun(@(x) x(:,3),data_Ctrl,'UniformOutput',0);
end
% activation group
load(sprintf('Functional Coupling Coding_1msbin_%dpairs_%s_ChR2Group.mat',length(PairsType),Target));
data_activation = AllTypesAUC{PairsID};
PairsNuminEachBin_activation = cellfun(@(x) size(x,1),data_activation,'UniformOutput',1);
if all(PairsNuminEachBin_activation >= 0)
...
if exist('FcspAucValue_inhibition') && exist('FcspAucValue_Ctrl') && exist('FcspAucValue_activation')
    ...
    box off;
    set(gcf,'Renderer','Painter'); saveas(gcf,['CompareFunctionalCouplingCoding_1msbin' Target '_' PairsType{PairsID} '_BetweenControlandOpto'],'fig'); close;
else
