...
for iUnit = 1:size(UnitsMeanDelayFR,1)
    ...
    if IsDVOrFRROC == 1
        ...
        [DV_Go,DV_Nogo] = CalculateDV(FRofWholeDelay_Go,FRofWholeDelay_Nogo);
        % step3: FPR and TPR
        [TPR,FPR] = CalculateTPRFPRValue(DV_Go,DV_Nogo);
    else
        % step3: FPR and TPR
        [TPR,FPR] = CalculateTPRFPRValue(FRofWholeDelay_Go',FRofWholeDelay_Nogo');
    end
    % step4: auROC
    TargetUnitsAUC(iUnit,1) = CalculateAUCValue(FPR,TPR);
end
