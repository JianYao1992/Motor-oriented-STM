%% Effect of removing FCSP events on STM-encoding ability (AUC) of following neurons
%% //////decreased STM-encoding ability (AUC) following removing FCSP events//////
...
%% Load result
load(sprintf('Decreased FU AUC with removing FCSP-related spikes-%s-%s-1msbin',Reg,Group));
...
AllFuAUC_MemtoMem = GetAUCofallFUsAfterRemovingFC(UniqFC_MemtoMem,ID_mPFC,mPFCSpikeTimeinIndividualTrial,mPFCUnitTrialMark,ID_aAIC,aAICSpikeTimeinIndividualTrial,...
    aAICUnitTrialMark);
% FC neuronal pairs: non-memory to memory
AllFuAUC_NonmemtoMem = GetAUCofallFUsAfterRemovingFC(UniqFC_NonmemtoMem,ID_mPFC,mPFCSpikeTimeinIndividualTrial,mPFCUnitTrialMark,ID_aAIC,aAICSpikeTimeinIndividualTrial,...
    aAICUnitTrialMark);
...
UniqFC_MemtoMem = cellfun(@(x,y) x(y<=0.1,:),UniqFC_MemtoMem,ChangedValue_MemtoMem,'UniformOutput',false);
UniqFR_MemtoMem = cellfun(@(x,y) x(y<=0.1,:),UniqFR_MemtoMem,ChangedValue_MemtoMem,'UniformOutput',false);
ChangedValue_MemtoMem = cellfun(@(x) x(x<=0.1,:),ChangedValue_MemtoMem,'UniformOutput',false);
UniqFC_NonmemtoMem = cellfun(@(x,y) x(y<=0.1,:),UniqFC_NonmemtoMem,ChangedValue_NonmemtoMem,'UniformOutput',false);
UniqFR_NonmemtoMem = cellfun(@(x,y) x(y<=0.1,:),UniqFR_NonmemtoMem,ChangedValue_NonmemtoMem,'UniformOutput',false);
ChangedValue_NonmemtoMem = cellfun(@(x) x(x<=0.1,:),ChangedValue_NonmemtoMem,'UniformOutput',false);
if IsPlot == 1
    % two-way ANOVA
    ...
    set(gcf,'Render','Painter'); saveas(gcf,sprintf('Decreased FU AUC with removing FCSP-related spikes-FRrange-%d-%d-%s-CtrlGroup',FRrange(1),FRrange(2),Reg),'fig'); close all;
    save(sprintf('Decreased FU AUC with removing FCSP-related spikes-FRrange-%d-%dHz-%s-%s-1msbin',FRrange(1),FRrange(2),Reg,Group),'ChangedValue_MemtoMem','ChangedValue_NonmemtoMem','UniqFC_MemtoMem','UniqFC_NonmemtoMem','UniqFR_MemtoMem','UniqFR_NonmemtoMem','tbl','-v7.3');
else
    save(sprintf('Decreased FU AUC with removing FCSP-related spikes-FRrange-%d-%dHz-%s-%s-1msbin',FRrange(1),FRrange(2),Reg,Group),'ChangedValue_MemtoMem','ChangedValue_NonmemtoMem','UniqFC_MemtoMem','UniqFC_NonmemtoMem','UniqFR_MemtoMem','UniqFR_NonmemtoMem','-v7.3');
end
