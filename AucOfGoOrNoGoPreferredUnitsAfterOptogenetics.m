%% Receiver operating characteristic(ROC) analysis for Go- and NoGo-preferred neurons 

clear; clc; close all;

%% Assignment
TargetBrain = 'aAIC';
UnitsType = 1; % 1 and 2 for Go- and NoGo-preferred neuron, respectively
...
load('Go-NoGo-PreferredUnitsID-CtrlGroup.mat'); % ID of Go- and NoGo-preferred neurons
switch TargetBrain
    case 'mPFC'
        PreferCodingNeuronID = PreferCodingNeuronID.mPFC;
    case 'aAIC'
        PreferCodingNeuronID = PreferCodingNeuronID.aAIC;
end
...
switch UnitsType
    case 1
        TarUnitsID = PreferCodingNeuronID.GoPrefUnitsID(:);
        savefilename = 'Go-preferred units ROC';
    case 2
        TarUnitsID = PreferCodingNeuronID.NoGoPrefUnitsID(:);
        savefilename = 'NoGo-preferred units ROC';
end

%% auROC of target neurons of control group
...
%% Load result of experimental group
load([TargetBrain 'FRinS1S2_' Group '.mat']); % FR
load(['Go-NoGo-PreferredUnitsID-' Group '.mat']); % ID of Go- and NoGo-preferred neurons
switch TargetBrain
    case 'mPFC'
        PreferCodingNeuronID = PreferCodingNeuronID.mPFC;
    case 'aAIC'
...
%% ID of target neurons of experimental group
switch UnitsType
    case 1
        TarUnitsID = PreferCodingNeuronID.GoPrefUnitsID(:);
    case 2
...
%% auROC of target neurons of experimental group
...
%% Plot auROC of target neurons, following projection-specific manipulation
figure;
...
hold on
errorbar(-0.5:1:size(TarUnitsAUC_Exp,2)-1.5,mean(TarUnitsAUC_Exp,1),std(TarUnitsAUC_Exp,0,1)/sqrt(size(TarUnitsAUC_Exp,1)),'color',C,'marker','o','markerfacecolor',C,'linewidth',3);
hold on
