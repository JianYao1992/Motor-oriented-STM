...
for type = 1:numel(PairsType)
    ...
    for bin = 1:BinNum
        FcPairs_Go = [];
        FcPairs_NoGo = [];
        load(['FCofNeurons_PropertyPopulation_' num2str(BinRange(bin)) '_' num2str(BinRange(bin)+BinSize) '_GoNogo' Group '.mat']);
        ...
        for iPair = 1:size(conn_chain_go,1)
            ...
            if max(Pref_Unit1) ~= min(Pref_Unit1) || max(Pref_Unit2) ~= min(Pref_Unit2)
...
%% Save variables
if IsConsiderNonsimulData == 1
    save(sprintf('Functional Coupling Coding_%dpairs_%s_%s',numel(PairsType),Target,Group),'AllTypesAUC','AllTypesFCPairID','AllUnSimulAUC','-v7.3');
else
    save(sprintf('Functional Coupling Coding_%dpairs_%s_%s',numel(PairsType),Target,Group),'AllTypesAUC','AllTypesFCPairID','-v7.3');
end
save(sprintf('Functional Coupling Firing Rate Difference_%dpairs_%s_%s',numel(PairsType),Target,Group),'DiffinFR','-v7.3');
