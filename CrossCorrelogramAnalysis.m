...
UnitsInfo = cell(size(UnitsSummary));
for i = 1:size(UnitsSummary,1)
    for j = 1:size(UnitsSummary,2)
        UnitsInfo{UnitsSummary{i,1}(1),j} = UnitsSummary{i,j};
    end
end
...
for iBin = -1:1:5 
    ...
    save(sprintf(['XCORR_%d_%d_1msbin_Projection_' Group '_GoNoGo.mat'],bin_range(1),bin_range(2)),'Sums','-v7.3'); %prefix
end
toc;
...
cfg.binsize     = 0.001; % bins of 2 ms
cfg.outputunit  = 'raw'; 
