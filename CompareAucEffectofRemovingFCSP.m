%% Compare effect of removing FCSP events on STM-encoding ability (AUC) of following neurons, following projection-specific manipulation.
...
%% Load result
% inhibition group
load(sprintf('Decreased FU AUC with removing FCSP-related spikes-%s-NpHRGroup.mat',Reg));
ChangedValue_MemtoMem_inhibition = ChangedValue_MemtoMem{1};
ChangedValue_NonmemtoMem_inhibition = ChangedValue_NonmemtoMem{1};
% control group
load(sprintf('Decreased FU AUC with removing FCSP-related spikes-%s-CtrlGroup.mat',Reg));
ChangedValue_MemtoMem_CtrlGroup = ChangedValue_MemtoMem{1};
ChangedValue_NonmemtoMem_CtrlGroup = ChangedValue_NonmemtoMem{1};
% activation group
load(sprintf('Decreased FU AUC with removing FCSP-related spikes-%s-ChR2Group.mat',Reg));
...
xlim([0.6 3.2]);
box off;
set(gcf,'Render','Painter'); saveas(gcf,sprintf('Compare decreased FU AUC with removing FCSP-MemtoMem-%s',Reg),'fig'); close all;
...
xlim([0.6 3.2]);
box off;
set(gcf,'Render','Painter'); saveas(gcf,sprintf('Compare decreased FU AUC with removing FCSP-NonmemtoMem-%s',Reg),'fig'); close all;
save(sprintf('Compare decreased FU AUC with removing FCSP-%s',Reg),'tb1_MemtoMem','tb1_NonmemtoMem','-v7.3');
