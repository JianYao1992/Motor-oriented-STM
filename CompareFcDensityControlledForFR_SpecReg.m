...
FRrangeID = 2; % 1: 2-7 Hz; 2: 7-13 Hz

%% Load result of FC density
% control group
load('Region-specific FC density within FR 2 to 13-CtrlGroup.mat');
FcDensity_Ctrl = [{IsSigFC_hit_ConAct},{IsSigFC_hit_ConInact},{IsSigFC_hit_InconAct},{IsSigFC_hit_InconInact},{IsSigFC_hit_Nonmem}];
% experimental group
load(sprintf('Region-specific FC density within FR 2 to 13-%s.mat',Group));
...
box off;
set(gcf,'Renderer','Painter'); saveas(gcf,['CompareFCdensity-PairsType' num2str(Type) '-' Reg '-BetweenCtrland' Group],'fig'); close;
