...
% because of rare miss trials in the ODR task, FR in hit, FA and CR without miss trials is analysed

clear; clc; close all;

%% Assignment
CortexName = 'aAIC';
PreferType = 1; % 1 and 2 for Go- and NoGo-preferred neurons, respectively
switch PreferType
    case 1
...
clearvars -except CortexName PreferType CodingType TimeGain FR_TarReg FR_Go FR_Nogo FR_Hit FR_False FR_CR
...
TriNumThres_FAandCR = [10 40];
BootstrapNum = 1000;
tempNeuID = find(UnitsGroupID(:,1) == PreferType & TrialsNum_FA(:) >= TriNumThres_FAandCR(1) & TrialsNum_CR(:) >= TriNumThres_FAandCR(2));
save(sprintf('ID-%s-%s_FA-%d-CR-%d',CortexName,CodingType,TriNumThres_FAandCR(1),TriNumThres_FAandCR(2)),'tempNeuID','-v7.3');
if numel(tempNeuID) > 1
    [Sam2CorrErrIsSig,Sam2CorrErrIsSig_BelowChance,FR_CorrErrTrials] = PlotSelecUnitsPerfDependFR(tempNeuID,FR_Hit,FR_CR,FR_FA,X,SampOdorLen,DelayLen,TestOdorLen,...
        RespLen,size(FR_TarReg{1,1},2),TimeGain,BasePeriod,BootstrapNum,[CodingType 'False>=' num2str(TriNumThres_FAandCR(1)) '-CR>=' num2str(TriNumThres_FAandCR(2))],IsClusBasedPermuTest,WorkerNum);
    set(gcf,'Render','Painter'); saveas(gcf,[CortexName '-' CodingType '-PerDependFR-False-' num2str(TriNumThres_FAandCR(1)) '-CR-' num2str(TriNumThres_FAandCR(2))],'fig'); close all;
    close all
end
