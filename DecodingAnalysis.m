...
%% Assignment
Target = 'aAIC';
NeuronCodingType = 0;  % 0: no remove; 1: remove sustained neurons; 2: remove transient neurons 3: only sustained 4: only transient 5: only non-memory neurons
switch(NeuronCodingType)
    case 0
        RegionCoding = [Target,' Neurons_All_'];
    case 1
        RegionCoding = [Target,' Neurons_Remove sustained_'];
    case 2
        RegionCoding = [Target,' Neurons_Remove transient_'];
    case 3
        RegionCoding = [Target,' Neurons_Only sustained_'];
    case 4
        RegionCoding = [Target,' Neurons_Only transient_'];
    case 5
        RegionCoding = [Target,' Neurons_Only nonmemory_'];
end
Group = 'CtrlGroup';
WorkerNumber = 10;
...
for iNullDis = 1:NullDistributionNum
    ...
    save([RegionCoding 'CVRecording-' num2str(iNullDis)], 'DECODING_RESULTS','-v7.3');
end
...
for iNullDis = 1:NullDistributionNum
    ...
    save([RegionCoding 'CVShuffled-' num2str(iNullDis)], 'ShuffleDECODING_RESULTS','-v7.3');
end
toc
...
if IsCTDecoding == 0
    ...
    save([RegionCoding '_CTDecoding_Shuffled' Group '_n=' num2str(UnitsNumUsedForDecoding)],'data','-v7.3');
end
