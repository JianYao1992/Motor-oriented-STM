...
for num = 1:FileNum
    ...
    for i=1:floor(size(GNGOutcome1,1)/TrialsNuminWindow)
        ...
        WindowPerf(i) = (HitNuminWindow(i)+CRNuminWindow(i))/TrialsNuminWindow;
    end
    if (floor(size(GNGOutcome1,1)/TrialsNuminWindow) ~= size(GNGOutcome1,1)/TrialsNuminWindow) & (size(GNGOutcome1,1)-TrialsNuminWindow*floor(size(GNGOutcome1,1)/TrialsNuminWindow) >= TrialsNuminWindow/2)
        ...
        WindowPerf(floor(size(GNGOutcome1,1)/TrialsNuminWindow)+1) =  (HitNuminWindow(floor(size(GNGOutcome1,1)/TrialsNuminWindow)+1)+CRNuminWindow(floor(size(GNGOutcome1,1)/TrialsNuminWindow)+1))/length(GNGOutcome1(TrialsNuminWindow*floor(size(GNGOutcome1,1)/TrialsNuminWindow)+1:end,1));
    end
...
for i = 1:size(Dataofmouse,2) % Day
    if iscell(Dataofmouse)
        ...
        WindowPerf{1,i} = Dataofmouse{1,i}.PerfinWindow;
        ...
        WindowPerf{1,i} = Dataofmouse.WindowPerf;
        WindowHitRate{1,i} = Dataofmouse.WindowHitRate;
        WindowCRRate{1,i} = Dataofmouse.WindowCRRate;
    end
end
save([CD(end-4:end) '_All_day_Performance'],'WindowPerf','WindowHitRate','WindowCRRate','GNGTrialMarker');
