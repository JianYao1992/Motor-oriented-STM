...
AllFuAUC_MemtoMem = GetAUCofallFUsAfterRemovingFCSPandNonFCSP(UniqFC_MemtoMem,ID_mPFC,mPFCSpikeTimeinIndividualTrial,mPFCUnitTrialMark,ID_aAIC,aAICSpikeTimeinIndividualTrial,...
    aAICUnitTrialMark);
% FC of non-memory to memory pairs
AllFuAUC_NonmemtoMem = GetAUCofallFUsAfterRemovingFCSPandNonFCSP(UniqFC_NonmemtoMem,ID_mPFC,mPFCSpikeTimeinIndividualTrial,mPFCUnitTrialMark,ID_aAIC,aAICSpikeTimeinIndividualTrial,...
    aAICUnitTrialMark);
...
ChangedValue_MemtoMem_RemoveFCSP = cellfun(@(x) (x(:,2)-x(:,1))./x(:,1),AllFuAUC_MemtoMem,'UniformOutput',0);
AverChangedValue_MemtoMem_RemoveFCSP = cellfun(@mean,ChangedValue_MemtoMem_RemoveFCSP);
ChangedValue_MemtoMem_RemoveNonFCSP = cellfun(@(x) (x(:,3)-x(:,1))./x(:,1),AllFuAUC_MemtoMem,'UniformOutput',0);
AverChangedValue_MemtoMem_RemoveNonFCSP = cellfun(@mean,ChangedValue_MemtoMem_RemoveNonFCSP);
ChangedValue_NonmemtoMem_RemoveFCSP = cellfun(@(x) (x(:,2)-x(:,1))./x(:,1),AllFuAUC_NonmemtoMem,'UniformOutput',0);
AverChangedValue_NonmemtoMem_RemoveFCSP = cellfun(@mean,ChangedValue_NonmemtoMem_RemoveFCSP);
ChangedValue_NonmemtoMem_RemoveNonFCSP = cellfun(@(x) (x(:,3)-x(:,1))./x(:,1),AllFuAUC_NonmemtoMem,'UniformOutput',0);
AverChangedValue_NonmemtoMem_RemoveNonFCSP = cellfun(@mean,ChangedValue_NonmemtoMem_RemoveNonFCSP);

%% Plot change in the AUC, between removing FCSP and non-FCSP events //////Two-way ANOVA///////
% FC with memory leading neuron
data_MemtoMem_RemoveFCSP = vertcat(ChangedValue_MemtoMem_RemoveFCSP{:});
data_MemtoMem_RemoveNonFCSP = vertcat(ChangedValue_MemtoMem_RemoveNonFCSP{:});
data = vertcat(data_MemtoMem_RemoveFCSP(:),data_MemtoMem_RemoveNonFCSP(:));
FCtype = [];
RemovedLuNum = [];
FCtype = vertcat(ones(numel(data_MemtoMem_RemoveFCSP),1),2*ones(numel(data_MemtoMem_RemoveNonFCSP),1));
temp = vertcat(ChangedValue_MemtoMem_RemoveFCSP,ChangedValue_MemtoMem_RemoveNonFCSP);
for iRow = 1:size(temp,1)
    for iColumn = 1:size(temp,2)
        RemovedLuNum = [RemovedLuNum; iColumn*ones(numel(temp{iRow,iColumn}),1)];
    end
end
[p,tb1_BetwFscpNoFcsp_MemtoMem] = anovan(data,{FCtype RemovedLuNum},'model',2,'varnames',{'FCtype','RemovedLUnum'},'display','on');
SemChangedValue_MemtoMem_RemoveFCSP = cellfun(@(x) std(x)/sqrt(numel(x)),ChangedValue_MemtoMem_RemoveFCSP); % plot figure
SemChangedValue_MemtoMem_RemoveNonFCSP = cellfun(@(x) std(x)/sqrt(numel(x)),ChangedValue_MemtoMem_RemoveNonFCSP);
for i = 1:numel(AverChangedValue_MemtoMem_RemoveFCSP)
    bar(1.1+1.8*(i-1),AverChangedValue_MemtoMem_RemoveFCSP(i),0.6,'facecolor',[0 0 0],'edgecolor',[0 0 0]); hold on
    bar(1.9+1.8*(i-1),AverChangedValue_MemtoMem_RemoveNonFCSP(i),0.6,'facecolor','none','edgecolor',[0 0 0]); hold on
    errorbar(1.1+1.8*(i-1),AverChangedValue_MemtoMem_RemoveFCSP(i),SemChangedValue_MemtoMem_RemoveFCSP(i),'k','marker','none'); hold on
    errorbar(1.9+1.8*(i-1),AverChangedValue_MemtoMem_RemoveNonFCSP(i),SemChangedValue_MemtoMem_RemoveNonFCSP(i),'k','marker','none'); hold on
end
set(gca,'XTick',1.5:1.8:2.5+1.8*(numel(AverChangedValue_MemtoMem_RemoveFCSP)-1),'XTickLabel',num2cell(1:1:6),'XLim',[0.5 2.5+1.8*(numel(AverChangedValue_MemtoMem_RemoveFCSP)-1)],'YTick',-1:0.05:0,'YTickLabel',num2cell(-100:5:0),'YLim',[min(data) 0]);
set(gcf,'Render','Painter'); saveas(gcf,sprintf('Decreased FU AUC with removing FCSP and Non-FCSP_memory leading neurons-%s-%s',Reg,Group),'fig'); close all;
% FC with non-memory leading neuron
data_NonmemtoMem_RemoveFCSP = vertcat(ChangedValue_NonmemtoMem_RemoveFCSP{:});
data_NonmemtoMem_RemoveNonFCSP = vertcat(ChangedValue_NonmemtoMem_RemoveNonFCSP{:});
data = vertcat(data_NonmemtoMem_RemoveFCSP(:),data_NonmemtoMem_RemoveNonFCSP(:));
FCtype = [];
RemovedLuNum = [];
FCtype = vertcat(ones(numel(data_NonmemtoMem_RemoveFCSP),1),2*ones(numel(data_NonmemtoMem_RemoveNonFCSP),1));
temp = vertcat(ChangedValue_NonmemtoMem_RemoveFCSP,ChangedValue_NonmemtoMem_RemoveNonFCSP);
for iRow = 1:size(temp,1)
    for iColumn = 1:size(temp,2)
        RemovedLuNum = [RemovedLuNum; iColumn*ones(numel(temp{iRow,iColumn}),1)];
    end
end
[p,tb1_BetwFscpNoFcsp_NonmemtoMem] = anovan(data,{FCtype RemovedLuNum},'model',2,'varnames',{'FCtype','RemovedLUnum'},'display','on');
SemChangedValue_NonmemtoMem_RemoveFCSP = cellfun(@(x) std(x)/sqrt(numel(x)),ChangedValue_NonmemtoMem_RemoveFCSP); % plot figure
SemChangedValue_NonmemtoMem_RemoveNonFCSP = cellfun(@(x) std(x)/sqrt(numel(x)),ChangedValue_NonmemtoMem_RemoveNonFCSP);
for i = 1:numel(AverChangedValue_NonmemtoMem_RemoveFCSP)
    bar(1.1+1.8*(i-1),AverChangedValue_NonmemtoMem_RemoveFCSP(i),0.6,'facecolor',[0 0 0],'edgecolor',[0 0 0]); hold on
    bar(1.9+1.8*(i-1),AverChangedValue_NonmemtoMem_RemoveNonFCSP(i),0.6,'facecolor','none','edgecolor',[0 0 0]); hold on
    errorbar(1.1+1.8*(i-1),AverChangedValue_NonmemtoMem_RemoveFCSP(i),SemChangedValue_NonmemtoMem_RemoveFCSP(i),'k','marker','none'); hold on
    errorbar(1.9+1.8*(i-1),AverChangedValue_NonmemtoMem_RemoveNonFCSP(i),SemChangedValue_NonmemtoMem_RemoveNonFCSP(i),'k','marker','none'); hold on
end
set(gca,'XTick',1.5:1.8:2.5+1.8*(numel(AverChangedValue_NonmemtoMem_RemoveFCSP)-1),'XTickLabel',num2cell(1:1:6),'XLim',[0.5 2.5+1.8*(numel(AverChangedValue_NonmemtoMem_RemoveFCSP)-1)],'YTick',-1:0.05:0,'YTickLabel',num2cell(-100:5:0),'YLim',[min(data) 0]);
set(gcf,'Render','Painter'); saveas(gcf,sprintf('Decreased FU AUC with removing FCSP and Non-FCSP_non-memory leading neurons-%s-%s',Reg,Group),'fig'); close all;

%% Save result
save(sprintf('Decreased FU AUC with removing spikes_FCSP and NonFCSP-%s-%s',Reg,Group),'ChangedValue_MemtoMem_RemoveFCSP','ChangedValue_NonmemtoMem_RemoveFCSP','ChangedValue_MemtoMem_RemoveNonFCSP','ChangedValue_NonmemtoMem_RemoveNonFCSP','FCpair_MemtoMem','FCpair_NonmemtoMem','FR_MemtoMem','FR_NonmemtoMem','tb1_BetwFscpNoFcsp_MemtoMem','tb1_BetwFscpNoFcsp_NonmemtoMem','-v7.3');
...
function AllFuAuc = GetAUCofallFUsAfterRemovingFCSPandNonFCSP(FCpair,mPFCUnitsID,mPFCUnitsSpikeTime,mPFCUnitsTrialMark,aAICUnitsID,aAICUnitsSpikeTime,aAICUnitsTrialMark)
...
for i = 1:numel(FCpair) % different numbers of leading neurons
    for j = 1:size(FCpair{i},1) % based on same following neurons
        ...
        [tempAUC_wiFC,tempAUC_woFC,tempAUC_woNonFC] = DecreaFUauROCofRemoveFCSP(ID_Bin,IDinReg_LU,IDinReg_FU,SpikeInfo_LU,SpikeInfo_FU,TrialMark,100);
        AllFuAuc{i} = [AllFuAuc{i}; horzcat(tempAUC_wiFC,tempAUC_woFC,tempAUC_woNonFC)];
    end
end
end

function [AUC_wiFC,AUC_woFC,AUC_woNonFC] = DecreaFUauROCofRemoveFCSP(BinID,IDinReg_PreUnit,IDinReg_PostUnit,RG_PreUnit,RG_PostUnit,TrialMark,RunTimes)
...
for iTrialType = 1:2
    ...
    for iTrial = 1:numel(RG_PostUnit)
        ...
        if iTrialType == 1
            ...
            FcspRate_Go = [FcspRate_Go; numel(tempRG_PostUnit)-numel(tempRG_PostUnit(KeptSpkId))];
        elseif iTrialType == 2
            FR_wiFC_NoGo = [FR_wiFC_NoGo; numel(tempRG_PostUnit)];
            FR_woFC_NoGo = [FR_woFC_NoGo; numel(tempRG_PostUnit(KeptSpkId))];
            FcspRate_NoGo = [FcspRate_NoGo; numel(tempRG_PostUnit)-numel(tempRG_PostUnit(KeptSpkId))];
        end
    end
end
...
%% Remove resampled non-FCSP events amd calculate AUC
AUC_woNonFC = zeros(RunTimes,1);
% parallel pool
WorkerNum = 10; 
...
for iRun = 1:RunTimes
    f(iRun) = parfeval(@RemoveNonFcspToCalculateAUC,1,FcspRate_Go,FcspRate_NoGo,FR_woFC_Go,FR_woFC_NoGo,FR_wiFC_Go,FR_wiFC_NoGo);
end
for iRun = 1:RunTimes
    [~,tempAUC_woNonFC] = fetchNext(f);
    AUC_woNonFC(iRun,1) = tempAUC_woNonFC;
end
AUC_woNonFC = 2*(max([0.5 mean(AUC_woNonFC)])-0.5);
end
