...
for bin = 1:length(BinRange)
    load(['FCofNeurons_PropertyPopulation_' num2str(BinRange(bin)) '_' num2str(BinRange(bin)+1) '_' Group '.mat']);
...
if strcmp(Target,'Local+Cross-region')
    ...
    save(sprintf('FcTemporalPattern_%s_%s_%s_%s.mat',Target,Neurontype,Group,WhichTrials),'FCinEachBin','FCinEachBin_mPFC','FCinEachBin_aAIC','FCinEachBin_Proj','-v7.3');
elseif strcmp(Target,'Within aAIC') || strcmp(Target,'Within mPFC') || strcmp(Target,'mPFC-aAIC')
    save(sprintf('FcTemporalPattern_%s_%s_%s_%s.mat',Target,Neurontype,Group,WhichTrials),'FCinEachBin_Tar','-v7.3');
end
...
ylim([0.5,max(ylim())]);
set(gcf,'Renderer','Painter'); saveas(gcf,[Neurontype '-' Target '-' Group '-' WhichTrials],'fig'); close;
