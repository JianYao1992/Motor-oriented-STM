...
function [NoTransientCodingNeuronID,tempSigBinID,tempZStatistics,tempP] = TransientCodingTest(NeuronID,FRinS1,FRinS2,PutativeSigBinID,BaseLen,SampOdorLen,DelayLen,TimeGain)

BinnedFiringRateinS1 = []; BinnedFiringRateinS2 = []; BinID = BaseLen+SampOdorLen+1:BaseLen+SampOdorLen+DelayLen; 
...
for iDelayBin = 1:length(BinID)
    BinnedFiringRateinS1(:,iDelayBin) = mean(FRinS1(:,TimeGain*(BaseLen+SampOdorLen+iDelayBin-1)+2:TimeGain*(BaseLen+SampOdorLen+iDelayBin)+1),2);
    BinnedFiringRateinS2(:,iDelayBin) = mean(FRinS2(:,TimeGain*(BaseLen+SampOdorLen+iDelayBin-1)+2:TimeGain*(BaseLen+SampOdorLen+iDelayBin)+1),2);
end
...
for i=1:length(PutativeDelaySigBinID) % putative sig bin ID
    for j=1:length(NoSigBinID) % no selectivity bin ID
        if tempP(NoSigBinID(j)-(BinID(1)-1),PutativeDelaySigBinID(i)-(BinID(1)-1)) <=0.05 | tempP(PutativeDelaySigBinID(i)-(BinID(1)-1),NoSigBinID(j)-(BinID(1)-1)) <=0.05
...
if isempty(SubPutativeDelaySigBinID)
    ...
    for i = 1:length(NoSigBinID)  % no selectivity bin ID
        for j = 1:length(SubPutativeDelaySigBinID) % selectivity bin ID
            if (tempP(NoSigBinID(i)-(BinID(1)-1),SubPutativeDelaySigBinID(j)-(BinID(1)-1))<=0.05 || tempP(SubPutativeDelaySigBinID(j)-(BinID(1)-1),NoSigBinID(i)-(BinID(1)-1))<=0.05)
